...
We’re building this product across 5 self-contained prompts; keep extending this repo and avoid deleting unrelated features.

This is prompt 1 of 5. **FSP‑1 — Foundation:** schema + RLS + Host onboarding + Event editor

---

You are implementing a deployed web app **from scratch or on top of whatever already exists in this Lovable project**. **Assume no chat history.** Follow **only** this message.
...
- Visibility: **Public** (searchable in Explore) vs **Unlisted** (never listed in Explore; only direct link).
- Editor shows **Free/Paid** toggle; **Paid** must be **disabled** with tooltip **“Coming soon”** (no real payments).
...
- RSVP requires sign‑in; signed‑out RSVP must redirect to auth then **return to the same event URL**.
...
- CSV export(s) with columns **name, email, RSVP status, check‑in time**; **UTF‑8 BOM**; opens in **Excel** and **Google Sheets**.
- **My Events** for any user with a role: aggregate events; filters: host, date range, text search; actions depend on role.

**Check‑in**

- Manual **ticket code** entry is sufficient (camera scan optional).
...
- All sensitive rules enforced **server‑side** (RLS / RPC), not UI‑only.
...
**RLS intent (must implement now at least for tables touched in FSP‑1; stub permissive policies ONLY where unavoidable and document follow‑ups)**
...
- **`checker`** role must not gain host‑only privileges (narrow checker policies come in FSP‑4).
...
4. **Public Host page** `/h/[slug]`:
   - Shows profile.
   - Lists **published + public** events for that host (**hide drafts** and **hide unlisted** from this listing).
5. **Events UI for Host role**:
   - Full editor fields listed above.
   - Draft/Published workflow + **Duplicate** (duplicate creates **draft**, clears attendee artifacts if copying from an existing event).
   - Visibility toggle.
   - **Free/Paid** toggle UI: **Paid disabled** + tooltip **Coming soon**; persist **free**.
